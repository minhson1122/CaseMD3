...
public class GameDataServlet extends HttpServlet {
    ...
    private void insertGame(HttpServletRequest request, HttpServletResponse response)
        ...
        String urlCover = request.getParameter("urlcover");
        String urlWallpaper = request.getParameter("urlwallpaper");
        Game newGame = new Game(title, price, description, genre, rating, developer, releaseDate, urlCover, urlWallpaper);
        try {
            if (!gameDAO.checkInfor(title,developer)){
                this.gameDAO.insertGame(newGame);
                request.setAttribute("message","Đã thêm thành công!");
                RequestDispatcher dispatcher = request.getRequestDispatcher("/game/create.jsp");
                dispatcher.forward(request, response);
            }else {
                request.setAttribute("message","Game đã tồn tại!");
                RequestDispatcher view = request.getRequestDispatcher("/game/create.jsp");
    ...
    private void updateGame(HttpServletRequest request, HttpServletResponse response)
        ...
        Game newGameUpdate = new Game(id,title, price, description, genre, rating, totalDownload, developer, releaseDate,urlCover,urlWallpaper);
