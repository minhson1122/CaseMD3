...
public class GameDAO implements IGameDAO {
    ...
    private static final String UPDATE_GAME_SQL = "update games set title = ?, price = ?,description = ?, genre = ?, rating = ?, totaldownload = ? , developer = ?, releasedate = ?, urlcover = ?, urlwallpaper = ? where id = ?";
    private static final String INSERT_GAME_SQL = "INSERT INTO games (title,price,description,genre,rating,developer,releasedate, urlcover, urlWallpaper) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);";
    private static final String SELECT_GAME_BY_ID = "select title,price,description,genre,rating,totaldownload,developer,releasedate, urlcover, urlWallpaper FROM games WHERE id =?";
    ...
    public void insertGame(Game game) {
        ...
        try (Connection connection = getConnect(); PreparedStatement preparedStatement = connection.prepareStatement(INSERT_GAME_SQL)) {
            ...
            preparedStatement.setString(8, game.getUrlCover());
            preparedStatement.setString(9, game.getUrlWallpaper());
    ...
    public Game selectGame(int id) {
                ...
                PreparedStatement preparedStatement = connection.prepareStatement(SELECT_GAME_BY_ID);)
        {
            ...
            if (rs.next()) {
                ...
                String urlCover = rs.getString("urlcover");
                String urlWallpaper = rs.getString("urlwallpaper");
                game = new Game(id, title, price, description, genre, rating, totalDownload, developer, releaseDate,urlCover, urlWallpaper);
    ...
    public List<Game> selectAllGame() {
        ...
        try (Connection connection = getConnect();
            ...
            while (rs.next()) {
                ...
                String urlCover = rs.getString("urlCover");
                String urlWallpaper = rs.getString("urlWallpaper");
                Game game = new Game(id, title, price, Description, Genre, Rating, Totaldownload, Developer, ReleaseDate, Purchased, isActive, urlCover, urlWallpaper);
                gamelist.add(game);
            }
        } catch (SQLException e) {
    ...
    public boolean updateGame(Game game) throws SQLException {
        ...
        try (Connection connection = getConnect(); PreparedStatement statement = connection.prepareStatement(UPDATE_GAME_SQL);) {
            ...
            statement.setString(9, game.getUrlCover());
            statement.setString(10, game.getUrlWallpaper());
            statement.setInt(11,game.getId());
    ...
    @Override
    public boolean checkInfor(String title, String developer) throws SQLException {

        try (Connection connection = getConnect();
             PreparedStatement statement = connection.prepareStatement("SELECT * FROM games WHERE title = '" + title + "' AND developer ='" + developer + "'");) {
